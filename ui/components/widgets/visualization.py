...
class TrainingThread(QThread):
    ...
    def __init__(self, classifier, num_epochs, start_epoch=0, checkpoint_settings=None):
        super().__init__()
        ...
        # 체크포인트 설정
        self.checkpoint_settings = checkpoint_settings or {'auto_save': True, 'save_frequency': 1}
    ...
    def run(self):
        ...
        try:
            ...
            for epoch in range(self.start_epoch, self.start_epoch + self.num_epochs):
                ...
                # 학습률 스케줄러 업데이트
                if hasattr(self.classifier, 'scheduler'):
                ...
                # 에폭 업데이트 시그널 발생
                self.update_progress.emit(epoch + 1 - self.start_epoch, train_loss, train_acc, val_loss, val_acc)
                
                # 체크포인트 자동 저장 설정 확인
                if self.checkpoint_settings['auto_save']:
                    # 저장 주기에 따라 체크포인트 저장
                    if (epoch + 1) % self.checkpoint_settings['save_frequency'] == 0:
    ...
    def save_checkpoint(self, epoch):
        """체크포인트 저장"""
        # 저장할 디렉토리가 없으면 생성
        os.makedirs(self.classifier.output_dir, exist_ok=True)
        
        # 체크포인트 파일 경로 - 일관된 형식 사용
        checkpoint_path = os.path.join(self.classifier.output_dir, f'checkpoint_epoch_{epoch}.pth')
        ...
        torch.save({
            ...
            'scheduler_state_dict': self.classifier.scheduler.state_dict() if hasattr(self.classifier, 'scheduler') else None,
